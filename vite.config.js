...
function buildPhotoPrompt({ handle, height, assembledWidth, towerSpecs, shot }) {
  ...
  return [
    ...
    `MANDATORY TOE-KICK CONSTRUCTION AND SCALE: preserve the complete recessed toe-kick below the bottom shelf across every tower bay. Its floor-to-bottom-shelf height is exactly 5 inches—no larger and no smaller—which is exactly ${(5 / Number(height) * 100).toFixed(2)}% of this ${height}-inch cabinet height. Treat that percentage as a measured image constraint: if the cabinet silhouette is H pixels tall, the underside of the bottom shelf must be only H × ${(5 / Number(height)).toFixed(5)} pixels above the cabinet floor line. It must read as a shallow architectural base, never as a storage cubby or tall plinth. Every bay must contain one SOLID, OPAQUE, WHITE MELAMINE VERTICAL KICK BOARD filling the entire shallow rectangular area from the floor up to the underside of the bottom shelf and from the left panel to the right panel. Set that solid board about 2 inches behind the cabinet front plane. It is a real cabinet part, not background wall: no wall, floor, open cavity, or empty darkness may be visible through the toe-kick zone. The side panels are not furniture legs. Lighting may create only soft natural depth shading on the solid kick board. Do not add a black line, dark stripe, trim strip, groove, gap, or separate horizontal product part at the toe kick. Do not enlarge the base for visibility, door clearance, trim, perspective, or composition.`,
    ...towerRules,
