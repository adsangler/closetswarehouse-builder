...
const standardCounts = { FR: 1, LH: 1, DH: 1, HS: 3, S3D: 4, H3D: 2, S2D: 4, S7: 6, S8: 6, S9: 6 };
...
// Shelf-only layouts are ordered bottom to top. With an even board count,
// use the upper of the two middle boards as the center fixed shelf.
export function shelfTowerFixedShelves(shelves) {
  const centerIndex = Math.floor(shelves.length / 2);
  return shelves.map((shelf, index) => ({
    ...shelf,
    fixed: index === centerIndex || index === shelves.length - 1,
  }));
}
