...
function getWidthOptions(configCode) {
  return ['S3D', 'H3D', 'S2D'].includes(configCode) ? [24, 30] : [18, 24, 30];
}
...
function buildTowerLayout(drawing, tower) {
  ...
  if (tower.code === 'DH') {
    ...
    return {
      ...
      rods: [
        { label: 'Upper rod', y: upperRodShelfY - rodDropBelowShelf, bayX: tower.bayX, width: tower.width },
...
function ClosetTypeStart({ onReachIn }) {
  return (
    <main className="grid min-h-screen place-items-center bg-brand-ui p-4 text-brand-black">
      <section className="w-full max-w-4xl rounded border border-stone-200 bg-white p-5 shadow-sm">
        <div className="mb-4">
